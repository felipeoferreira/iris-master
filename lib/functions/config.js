...
const Broad_Files = async (isFiler, canCaption, kill, it, encryptMedia, pushname, body, cmd) => {
	if (isFiler) {
		...
		const extension = mime.extension(encryptMedia.mimetype)
		const filename = 'file.' + extension
		if (canCaption) {
			await kill.sendFile(it, tools('others').dataURI(encryptMedia.mimetype, mediaData), filename, `[Transmissão de ${pushname} ]\n\n${body.slice(cmd.length+6)}`);
		} else {
			await kill.sendFile(it, tools('others').dataURI(encryptMedia.mimetype, mediaData), filename, '');
...
exports.kconfig = async (fun, mesg) => {
	...
	try {
		...
		procTime = procTime < 0 ? '0.1-' : procTime; /* Velocidade de recebimento pode ser bem menor do que 0.001, mas defini como algo no mínimo 'aceitável' */
		...
		if (isCmd && tools('cooldown').isFiltered(chatId) && !isGroupMsg && !isOwner && config.Show_Logs == true){
			await kill.reply(chatId, "num vo fazer nao fds  " , id); 
			console.log(tools('others').color('> [FLOOD AS]', 'red'), tools('others').color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'yellow'), tools('others').color(`"[${prefix}${command.toUpperCase()}] [${args.length}]"`, 'red'), 'DE', tools('others').color(`"${pushname} - [${user.replace('@c.us', '')}]"`, 'red'));
			return
		}
		...
		if (isCmd && tools('cooldown').isFiltered(chatId) && isGroupMsg && !isOwner && config.Show_Logs == true){
			await kill.reply(chatId, "num vo fazer nao fds ", id);
			console.log(tools('others').color('> [FLOOD AS]', 'red'), tools('others').color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'yellow'), tools('others').color(`"[${prefix}${command.toUpperCase()}] [${args.length}]"`, 'red'), 'DE', tools('others').color(`"${pushname} - [${user.replace('@c.us', '')}]"`, 'red'), 'EM', tools('others').color(`"${name}"`));
			return
		}
		...
		if (isActivated('autosticker') && isType('image') && !isCmd && !isBot && encryptMedia && Object.keys(quotedMsgObj).length == 0) {
			const mediaData = await decryptMedia(encryptMedia);
		...
		if (!tools('cooldown').isFiltered(chatId) && !isCmd) {
			try {
				if (body.toLowerCase().includes('removeu voc')) {
					await kill.reply(chatId, 'Anti-troll foi ativado.\nBanindo voc em... 3.. 2.. 1..\nAdeus!', id);
				}

				if (body.toLowerCase().includes("@557183809633")) {
					await kill.reply(from, '*DEIXA O MEU DONO EM PAZ* \n~ele  baianor, deve ta dormindo~', id);
				}

				if (body.toLowerCase().includes('bot ')){
					await kill.reply(from, '*Eu morri seu arrombado*, me deixa em paz.\n*Vou te bloquear, OTARIO!*', id);
				}

				if (body.toLowerCase().includes("@553185530569")) {
					await kill.reply(
						from, tools('others').randVal([
							'_Eu dei um jeito no Leo, e o proximo pode ser vc._ ',
							'_Temporariamente indisponivel, tente mais tarde._\n*Obrigado!*',
							'_Sua mensagem foi encaminhada para caixa postal._\n_Apos o sinal deixe seu recado._\n*tuuuuuuuu*'
						]), id
					);
				}

				if (body.toLowerCase().includes("@554791135229")) {
					await kill.reply(from, '*Ala o comedor de vacas*\n KKKKKKKKKKKKKKKK', id);
				}
			} catch (error) {
				console.log(error.message)
			}
...
/*				case 'newprefix':
				...
				case 'porn':
					...
					let imageGIF = await nsfw.pgif();
					if (imageGIF.endsWith('.gif')) {
						await tools('ffmpeg').resize(imageGIF, cmd, kill, message);
					} else await mandarFig(kill, chatId, imageGIF, stickerConfig);
				break;
				...
				case 'meme':
					if (isType('image') && args.length >= 2 && arks.includes('|')) {
						const mediaData = await decryptMedia(encryptMedia);
						const Uploaded_Meme = await tools('cloud').upload(mediaData);
				...
				case 'addcmd':
					...
					for (let i = 0; i < Object.keys(commandForm).length; i++) {
						const propedVal = Object.keys(commandForm)[i];
						if (newCmdToAdd.cancel) return;
						const Temp_MID = await kill.reply(chatId, `Marque essa mensagem com ${commandForm[propedVal]}. Para cancelar o comando a qualquer momento, marque essa mensagem com 'cancel'`, id);
						const filtercmd = msgw => {
							if (msgw.quotedMsg !== null && msgw.sender.id == user && msgw.from == chatId && Temp_MID.includes(msgw.quotedMsg.id)) {
								return true;
							} else return false;
						};
						kill.awaitMessages(chatId, filtercmd, {
							max: 1,
							time: 300000,
							errors: ['time']
						}).then(async collected => {
							if (/cancel/gi.test(removeAccents(Array.from(collected)[0][1].text))) {
								newCmdToAdd.cancel = true;
								return await kill.reply(chatId, mess.youCanceled(), id);
							} else if (/none/gi.test(removeAccents(Array.from(collected)[0][1].text))) {
								newCmdToAdd[propedVal] = '';
							} else {
								newCmdToAdd[propedVal] = Array.from(collected)[0][1].text;
							}
					...
					if (newCmdToAdd.cancel) return;
					const Phrasetwo = ["alias", "media", "menus"];
					for (let i = 0; i < Phrasetwo.length; i++) {
						const Proppt = Phrasetwo[i];
						if (newCmdToAdd[Proppt].includes('|')) {
							let array = [];
							for (let string of newCmdToAdd[Proppt].split('|')) {
								array.push(string);
							}
							newCmdToAdd[Proppt] = array;
						}
					}
					const Phrasethree = ["description", "obs"];
					for (let i = 0; i < Phrasethree.length; i++) {
						const PropEt = Phrasethree[i];
						if (newCmdToAdd[PropEt].includes('|')) {
							let langs = {};
							langs.en = newCmdToAdd[PropEt].split('|')[0];
							langs.es = newCmdToAdd[PropEt].split('|')[1];
							if (newCmdToAdd[PropEt].split('|')[2]) {
								langs.pt = newCmdToAdd[PropEt].split('|')[2];
							}
							newCmdToAdd[PropEt] = langs;
						}
					}
